...
```{r knitr.settings, eval=T, echo=F, results = "hide"}
...
rm -rf Silva.nr_v119.tgz silva.nr_v119.tax Anaconda-2.3.0-MacOSX-x86_64.sh
```
...
```{r}
otu_table_biom <- read_biom("otu_table_tax_final.biom")
otu_table <- as_data_frame(as(biom_data(otu_table_biom), "matrix"))
...
otu_table_trans <- as_data_frame(t(otu_table))
